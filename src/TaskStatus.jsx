...
export default function TaskStatus({inProgress,resolved, handleComplete}) {
  return (
    ...
    <div className='md:w-full max-w-full bg-base-200 p-4 rounded-xl'>
      ...
      <h2 className='font-semibold mb-2'>Task Status</h2>
      {
        inProgress.map(task => (
           <div key={task.id}  className='bg-amber-100 shadow-xl mb-2 p-3'> 
            ...
            <button className='btn btn-success btn-sm mt-2 w-full sm:w-auto' onClick={() => handleComplete(task)} >Complete</button>
           </div> 
        ))
      }
      ...
      {
        resolved.map(task => (
            <div key={task.id} className='mt-2 text-sm text-gray-600 break-words'>
